...
export default function Puzzle2048Game({ onBack, themeColor }: Puzzle2048GameProps) {
  ...
  const getTileColor = (value: number | null) => {
    ...
    const colors: { [key: number]: string } = {
      2: "bg-secondary text-card-foreground",
      4: "bg-accent text-card-foreground",
    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-100 p-4">
      <div className="max-w-md mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <Button onClick={onBack} variant="outline" size="sm" className="bg-card/80 backdrop-blur-sm hover:bg-card">
            <ArrowLeft className="w-4 h-4 mr-2" />
            Back
          </Button>
          <h1 className="text-2xl font-bold text-card-foreground">2048</h1>
          <Button
            onClick={resetGame}
            variant="outline"
            size="sm"
        ...
        <div className="flex gap-4 mb-6">
          <div className="bg-card rounded-lg p-3 flex-1 text-center shadow-sm">
            <div className="text-sm text-muted-foreground">Score</div>
            <div className="text-xl font-bold text-card-foreground">{score.toLocaleString()}</div>
          </div>
          <div className="bg-card rounded-lg p-3 flex-1 text-center shadow-sm">
            <div className="text-sm text-muted-foreground">Best</div>
            <div className="text-xl font-bold text-card-foreground">{bestScore.toLocaleString()}</div>
          </div>
          <div className="bg-card rounded-lg p-3 flex-1 text-center shadow-sm">
            <div className="text-sm text-muted-foreground">Moves</div>
            <div className="text-xl font-bold text-card-foreground">{moveCount}</div>
          </div>
        </div>
        ...
        <div className="text-center mb-6">
          <p className="text-muted-foreground text-sm mb-4">Use arrow keys or WASD to move tiles</p>
        ...
        {(gameOver || won) && (
          <div className="fixed inset-0 bg-foreground/50 flex items-center justify-center z-50">
            <div className="bg-card rounded-lg p-6 max-w-sm mx-4 text-center shadow-xl">
              <div className="mb-4">
                {won ? (
                ...
                <h2 className="text-2xl font-bold text-card-foreground">{won ? "You Win!" : "Game Over"}</h2>

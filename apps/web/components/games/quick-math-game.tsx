...
export default function QuickMathGame({ onBack, themeColor = "#111827" }: Props) {
  ...
  return (
    <div
      className="w-full h-full min-h-screen bg-card flex flex-col items-center justify-start pt-20 px-4"
      style={accentStyle}
    >
      <div className="w-full max-w-xl space-y-6">
        <div className="flex items-center justify-between">
          <Button onClick={onBack} variant="outline" className="bg-card">
            Back
          </Button>
          <div className="text-sm text-muted-foreground">
            Best: <span className="font-semibold text-card-foreground">{best}</span>
          </div>
        </div>
        ...
        <Card className="p-6 text-center">
          <div className="text-sm text-muted-foreground mb-2">Solve</div>
            ...
            return (
              <button
                ...
                className={`rounded-xl border p-4 text-xl font-semibold transition-all
                  ...
                  ${selected === null ? "bg-card hover:bg-secondary border-border text-card-foreground" : ""}
                `}
